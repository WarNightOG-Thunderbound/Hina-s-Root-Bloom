...
document.addEventListener('DOMContentLoaded', () => {
    ...
    // --- Admin Authentication (from public app) ---
    if (adminLoginLink) {
    ...
    if (adminLoginBtn) {
        adminLoginBtn.addEventListener('click', () => {
            ...
            if (!email || !password) {
                if (adminLoginError) adminLoginError.textContent = "Please enter both email and password.";
                return;
            }
            ...
            signInWithEmailAndPassword(auth, email, password)
                ...
                .catch((error) => {
                    ...
                    if (adminLoginError) adminLoginError.textContent = "Login failed: Invalid email or password.";
                });
        });
    }
    ...
    function displayProducts(productsToDisplay) {
        if (productGrid) productGrid.innerHTML = ''; // Clear current products
        if (productsToDisplay.length === 0) {
            if (productGrid) productGrid.innerHTML = '<p class="no-results">No products found matching your criteria.</p>';
            return;
        }
    ...
    function populateCategoryFilters(productsData) {
        ...
        categories.forEach(category => {
            ...
            if (categoryFiltersContainer) categoryFiltersContainer.appendChild(button);
        });
    ...
    function openProductDetailModal(productId) {
        ...
        if (modalProductImage) modalProductImage.src = product.imageUrl || 'placeholder.png';
        ...
        if (modalProductCategory) modalProductCategory.textContent = `Category: ${product.category || 'N/A'}`;
        if (modalProductDescription) modalProductDescription.textContent = product.description;

        // Handle video display
        if (modalProductVideo) {
            if (product.videoUrl) {
        ...
        const averageRating = calculateAverageRating(product.id, product.ratings);
        if (modalProductRating) modalProductRating.innerHTML = `Average Rating: ${generateStarRating(averageRating)} (${Object.keys(product.ratings || {}).length} reviews)`;
    ...
    window.addEventListener('click', (event) => {
        ...
        } else if (orderFormModal && event.target === orderFormModal) {
            orderFormModal.style.display = 'none';
        } else if (ratingModal && event.target === ratingModal) {
            closeRatingModal();
    ...
    if (placeOrderDetailBtn) {
        placeOrderDetailBtn.addEventListener('click', () => {
            if (currentProductToOrder) {
                if (orderFormModal) orderFormModal.style.display = 'flex';
                if (productDetailModal) productDetailModal.style.display = 'none'; // Close detail modal
                if (orderAddressInput) orderAddressInput.value = ''; // Clear previous input
                if (orderPhoneInput) orderPhoneInput.value = ''; // Clear previous input
            } else {
    ...
    if (submitOrderBtn) {
        submitOrderBtn.addEventListener('click', () => {
            const address = orderAddressInput ? orderAddressInput.value.trim() : '';
            const phoneNumber = orderPhoneInput ? orderPhoneInput.value.trim() : '';
    ...
    function openRatingModal(productId) {
        ...
        const product = products.find(p => p.id === productId);
        if (product) {
            if (ratingProductTitle) ratingProductTitle.textContent = product.title;
            selectedRating = 0; // Reset selected rating
            updateRatingStars();
            if (ratingError) ratingError.textContent = ''; // Clear any previous error
    ...
    if (submitRatingButton) submitRatingButton.addEventListener('click', submitProductRating);
    ...
    function submitProductRating() {
        if (!currentProductToRate) {
            if (ratingError) ratingError.textContent = "No product selected for rating.";
            return;
        }
        if (selectedRating === 0) {
