...
updateProductBtn.addEventListener('click', async () => {
    ...
    const videoUrl = editProductVideoUrl.value.trim();
    ...
    onValue(categoriesRef, (snapshot) => {
        ...
        if (snapshot.exists()) {
            snapshot.forEach((childSnapshot) => {
                ...
                editProductCategory.appendChild(optionEdit);
            });
        }
...
function populateProductSelects(productsData) {
    ...
    productRatingSelect.innerHTML = '<option value="">Select a Product</option>';
    ...
    productsData.forEach(product => {
        ...
        productRatingSelect.appendChild(optionRating);
        ...
        compareProduct1Select.appendChild(optionCompare1);
        ...
        compareProduct2Select.appendChild(optionCompare2);
    });
}
...
function updateRatingsAnalytics(productId) {
    ...
    if (!product || !product.ratings) {
        ratingDistributionList.innerHTML = '<li>No ratings for this product.</li>';
        return;
    }
    ...
    for (let i = 5; i >= 1; i--) {
        ...
        ratingDistributionList.appendChild(listItem);
    }
}
